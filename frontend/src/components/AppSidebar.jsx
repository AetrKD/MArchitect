...
function AppSidebar({ path, role, language, theme, onLanguageChange, onThemeToggle, onNavigate, onLogout }) {
  ...
  const subtitle = role === "admin" ? `${sidebar.admin.toUpperCase()} CONSOLE` : `${sidebar.user.toUpperCase()} CONSOLE`;
  ...
  return <aside className={`admin-sidebar ${isOpen ? "is-open" : ""}`}>
    ...
    <nav aria-label="Main navigation">{navigation.map(([target, label, icon]) => <button className={path === target ? "active" : ""} type="button" key={target} onClick={() => navigateAndClose(target)}><span aria-hidden="true">{icon}</span>{label}</button>)}</nav>
    <div className="sidebar-preferences"><label htmlFor="language-select">{sidebar.language}</label><select id="language-select" value={language} onChange={(event) => onLanguageChange(event.target.value)}><option value="ko">한국어</option><option value="en">English</option></select><button type="button" onClick={onThemeToggle}>{theme === "dark" ? `☀ ${sidebar.lightMode}` : "◐ Dark mode"}</button><button className="logout-button sidebar-logout" type="button" onClick={logoutAndClose}>{common.logOut}</button></div>
  </aside>;
}
